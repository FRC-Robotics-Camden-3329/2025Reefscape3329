...
public class AutoDriveToReefCommand extends Command {
   ...
   */
  public AutoDriveToReefCommand(Pose2d goal, SwerveSubsystem swerveSubsystem) {
  ...
  public void initialize() {
    // first need to know where we are trying to drive to
    Pose2d dest = swerveSubsystem.getReefTransformation(goal, swerveSubsystem.getNearestSide());

    // the path will error if the drivetrain is too close to the destination
    if (swerveSubsystem.getSwerveDrive().getPose().getTranslation().getDistance(dest.getTranslation()) < 0.05) {
      // so just do nothing if the path is too close
      DataLogManager.log("Unable to AutoDrive: Too Close");
      tooCloseAlert.set(true);
      ...
      waypoints = PathPlannerPath.waypointsFromPoses(swerveSubsystem.getSwerveDrive().getPose(), dest);
